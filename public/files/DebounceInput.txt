<template>
  <div class="control has-icons-left" :class="{ 'has-icons-right': hasDetailsIcon }">
    <input 
      class="input" 
      :class="{ 'has-icons-right': hasDetailsIcon }" 
      type="text" 
    ...
    <span v-if="hasDetailsIcon" class="icon is-right is-clickable details-icon">
...
const handleInput = (event: Event) => {
  const inputValue =  triggerDebounce((event.target as HTMLInputElement).value)
